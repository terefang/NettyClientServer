...
{
    public static final String PREFIX_BCRYPT = "$2";
            ...
            byte[] psw)
    {
        if (salt.length != 16)
        {
            throw new IllegalArgumentException("Invalid salt size: 16 bytes expected.");
        }
        if (cost < 4 || cost > 31)
        {
        ...
        bigEndianToInt(salt, 0, salt32Bit);
     ...
     */
    public static byte[] generateHash(byte[] password, byte[] salt, int cost)
    {
    ...
    public static void fill(int[] buf, int val)
    {
    ...
    public static void fill(byte[] buf, int val)
    {
        for(int i=0; i<buf.length; i++)
        {
            buf[i] = (byte) (val & 0xff);
                                             ...
                                             int cost)
    {
        ...
        byte[] key = BCrypt.generateHash(password, salt, cost);
    ...
    {
        return generate(defaultVersion, password, salt, 12);
    }

    @SneakyThrows
    public static String generate( String password, byte[] salt)
    {
        return generate(defaultVersion, password.toCharArray(), salt, 12);
    }
    @SneakyThrows
            ...
            int cost)
    {
        ...
        {
            throw new IllegalArgumentException("16 byte salt required: " + salt.length);
        }
        ...
        byte[] psw = (new String(password)).getBytes(StandardCharsets.UTF_8);
     ...
     */
    public static boolean checkPassword( String bcryptString, String password)
    {
        return checkPassword(bcryptString, password.toCharArray());
    }
    ...
    public static boolean checkPassword( String bcryptString, char[] password)
    {
        ...
        {
            throw new IllegalArgumentException("Bcrypt String length: "
...

    {
        ...
        {
            throw new IllegalArgumentException("Invalid length: " + data.length + ", 24 for key or 16 for salt expected");
        }
        boolean salt = false;
            ...
            String saltString)
    {
        ...
        {
            throw new IllegalArgumentException("Invalid base64 salt length: " + saltChars.length + " , 22 required.");
        }
